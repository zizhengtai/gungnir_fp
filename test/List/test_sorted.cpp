...
TEST_CASE("test List sorted", "[List][sorted]") {
    ...
    SECTION("empty List") {
        ...
        REQUIRE(xs.sorted(true).isEmpty());
        REQUIRE(xs.sorted(gtI, true).isEmpty());

        List<PI> ys;
        REQUIRE(ys.sorted(ltP).isEmpty());
        REQUIRE(ys.sorted(gtP).isEmpty());
        REQUIRE(ys.sorted(ltP, true).isEmpty());
        REQUIRE(ys.sorted(gtP, true).isEmpty());
    }
    SECTION("List with one element") {
        List<int> xs(123);
        REQUIRE(xs.sorted() == List<int>(123));
        REQUIRE(xs.sorted(gtI) == List<int>(123));
        REQUIRE(xs.sorted(true) == List<int>(123));
        REQUIRE(xs.sorted(gtI, true) == List<int>(123));

        List<PI> ys(PI(new int(456)));;
        REQUIRE(ys.sorted(ltP) == ys);
        REQUIRE(ys.sorted(gtP) == ys);
        REQUIRE(ys.sorted(ltP, true) == ys);
        REQUIRE(ys.sorted(gtP, true) == ys);
    ...
    SECTION("stable sort") {
        ...
        for (std::size_t i = 0; i < yss.size(); ++i) {
            ...
            REQUIRE(yss[i].sorted(false).map(&Foo::val) == xss[i]);
        }
    }
}
